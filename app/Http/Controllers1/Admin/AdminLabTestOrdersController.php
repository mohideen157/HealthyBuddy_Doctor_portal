...
class AdminLabTestOrdersController extends Controller
{
    public function index(){
		$orders = Order::where('order_type', 'lab_test')->orderBy('id', 'desc')->get();

		return view('admin.orders.lab-tests.index', [
			'orders' => $orders
		]);
	}
	...
	public function show($id){
		...
		if (!$order || $order->order_type != 'lab_test') {
			return redirect('/admin/lab-test-orders')->with('error', 'Order not found');
		}
		...
		return view('admin.orders.lab-tests.show', [
			'order' => $order,
			'user' => $order_user,
	...
	public function update(Request $request, $id){
		...
		$sendemail = Mail::send('emails.orderStatusChange', array('name' => $user->name, 'order_id' => $order->shdct_id, 'order_type' => 'lab test', 'status' => $request->status), function ($message) use ($user)
		{
			$message->to($user->email, $user->name);
			$message->subject('SheDoctr - Order Status');
		});

		return redirect('/admin/lab-test-orders')->with('status', 'Order Updated');
	}
