...
class ConsultationTimeController extends Controller
{
	...
	public function index(){
		try{
			...
			$timeslots = DoctorTimeSlots::where('doctor_id', $id)->whereRaw('date >= curdate()')->whereNotIn('id', $invalid_slots)->orderBy('date', 'asc')->orderBy('time_start', 'asc')->get();

			foreach ($timeslots as $value) {
			
				$timestamp = strtotime( $value->date . ' ' .$value->time_start );
				//$timestamp =$value->date . ' ' .$value->time_start ;
